...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $checkLevel1Stmt = $conn->prepare("SELECT highest_score FROM quizresults WHERE username = ? AND quiz_topic = ? AND quiz_level = 1");
    $checkLevel1Stmt->bind_param("ss", $username, $quizType);
    $checkLevel1Stmt->execute();
    $level1Result = $checkLevel1Stmt->get_result();
    
    $canAttemptLevel2 = false;
    if ($level1Result->num_rows > 0) {
        $level1Row = $level1Result->fetch_assoc();
        if ($level1Row['highest_score'] >= 4) {
            $canAttemptLevel2 = true;
        }
    }
    ...
    if (!$canAttemptLevel2) {
        ...
        echo '</div>';
        echo '<div class="que">';
        echo '<h2>You need to score 4 or more in Level 1 of ' . htmlspecialchars($quizType) . ' quiz to attempt this level.</h2>';
        echo '</div>';
        echo '<div class="qz1">';
        echo '<div class="qbtn">';
    ...
    $stmt = $conn->prepare("SELECT * FROM quiz_questions WHERE quiz_type = ? AND quiz_level = ? ORDER BY RAND() LIMIT 15");
    ...
    if ($result->num_rows > 0) {
        echo '<div class="quizstart">';
        echo '<h1 id="heading">'.strtoupper($quizType).' QUIZ - LEVEL 2</h1>';
        echo '</div>';
        ...
        echo '</div>';
        echo '<div class="que">';
        echo '<h2>No level 2 questions available for this quiz type.</h2>';
        echo '</div>';
        echo '<div class="qz1">';
        echo '<div class="qbtn">';
        echo '<a href="index.php"><button class="qbtnexit">BACK TO HOME</button></a>';
    ...
    $totalQuestions = $_POST['total_questions'] ?? 15; 
